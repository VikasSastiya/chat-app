...
export default function FriendSearch() {
    ...
    const getUserStatus = (user: User) => {
        ...
        if (user.friendRequestStatus?.status === "PENDING") {
            return user.friendRequestStatus.senderId === user.id
                ? "received"
                : "sent";
        }
                    ...
                    {(error || successMessage) && (
                        ...
                        >
                            {error && (
                                <Alert
                                    variant="destructive"
                                    className="bg-red-50 border-l-4 border-red-500 rounded-lg shadow-md"
                                >
                                    <AlertCircle className="w-4 h-4 md:w-5 md:h-5 mr-2 text-red-500" />
                                    <AlertDescription className="text-sm md:text-base text-red-700 font-medium">
                                        {error}
                                    </AlertDescription>
                                </Alert>
                            )}
                                        ...
                                        <Card className="overflow-hidden shadow-md hover:shadow-lg transition-all duration-300">
                                            <CardContent className="p-3 md:p-6 flex items-center justify-between">
                                                <div className="flex items-center gap-2 md:gap-4">
                                                    ...
                                                    <div>
                                                        <p className="font-semibold text-sm md:text-lg text-gray-800 dark:text-gray-200">
                                                            {user.name}
                                                        </p>
                                                        {user.email && (
